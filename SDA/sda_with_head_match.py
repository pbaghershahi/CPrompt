...
for _ in range(5):
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(t_dataset.train_loader):
            ...
            # print("pred labels size: ", pred_label.size())
            ...
            pos_out, _ = main_model(initc[pred_label, :], decoder=False)
            neg_x_adj = batch_to_xadj_list(neg_batch, device)
            ...
            neg_dists = neg_embed @ initc.T / \
             (neg_norm[:, None].tile(1, initc.size(0)) * initc_norm[None, :].tile(neg_embed.size(0), 1))
            # dd = cdist(ps_embds, initc, 'cosine')
            neg_label = neg_dists.argmax(dim=1)
            neg_out, _ = main_model(initc[neg_label, :], decoder=False)
            loss = ntxent_loss(prompt_out, pos_out.detach().clone(), neg_out.detach().clone(), weighting=None)
            if add_link_loss:
                adj_labels = get_adj_labels(batch).to(device)
                loss += link_predict_loss(prompt_batch, adj_labels)
            # print("pps out: ", pos_out.size())
