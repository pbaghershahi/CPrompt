def ntxent_loss(
    ...
    if pos_labels is not None:
        pos_labels = pos_labels[:, None].tile(1, pos_labels.size(0))
        neg_labels = neg_labels[None, :].tile(neg_labels.size(0), 1)
        gt_neg_labels = (neg_labels != pos_labels)
        min_neg = max(gt_neg_labels.sum(dim=1).min(), min(torch.tensor([10]), neg.size(1)))
        idx_mask = torch.arange(neg.size(1)).tile(neg.size(0), 1)
        idx_mask = torch.where(gt_neg_labels, idx_mask, -1)
        idx_mask = idx_mask.sort(dim=1, descending=True)[0][:, :min_neg]
        replace_mask = torch.randint(min_neg, (neg.size(0), min_neg))
        idx_mask = torch.where(idx_mask != -1, idx_mask, replace_mask)
...
for _ in range(5):
    ...
    for epoch in range(n_epochs):
        ...
        for i, batch in enumerate(t_dataset.train_loader):
            ...
            pos_embds = (pos_embds.T / pos_embds.norm(p=2, dim=1)).T
            ps_embds = pos_embds.detach().cpu().numpy()
            ps_probs = pos_probs.detach().cpu().numpy()
            initc = ps_probs.T @ ps_embds
            initc = initc / (1e-8 + ps_probs.sum(axis=0)[:,None])
            pos_dd = cdist(ps_embds, initc, 'cosine')
            pos_preds = pos_dd.argmin(axis=1)

            for round in range(2):
                ps_probs = np.eye(ps_probs.shape[1])[pos_preds, :]
                initc = ps_probs.T @ ps_embds
                initc = initc / (1e-8 + ps_probs.sum(axis=0)[:,None])
                pos_dd = cdist(ps_embds, initc, 'cosine')
                pos_preds = pos_dd.argmin(axis=1)
            ...
            neg_norm = neg_embed.norm(p=2, dim=1)
            neg_norm = torch.max(neg_norm, torch.ones_like(neg_norm)*1e-8)
            neg_embed = (neg_embed.T / neg_norm).T
            ng_embds = neg_embed.detach().cpu().numpy()
            neg_dd = cdist(ng_embds, initc, 'cosine')
            neg_preds = pos_dd.argmin(axis=1)
            pos_preds = torch.as_tensor(pos_preds)
            neg_preds = torch.as_tensor(neg_preds)
            acc = (pos_preds == labels).sum()/pos_preds.size(0)
            # print("Acc: ", acc.item())
            initc = torch.as_tensor(initc, dtype=torch.float, device=prompt_out.device)
            pos_cc_out, _ = main_model(initc[pos_preds, :], decoder=False)
            neg_cc_out, _ = main_model(initc[neg_preds, :], decoder=False)
            # loss = ntxent_loss(prompt_out, pos_cc_out.detach().clone(), neg_cc_out.detach().clone(), weighting=None)
            # print(prompt_out.size(), pos_out.size(), neg_out.size())
            loss = ntxent_loss(
                prompt_out,
                pos_cc_out,
                neg_cc_out,
                # prompt_embds,
                # initc[pos_preds, :].detach().clone(),
                # initc[neg_preds, :].detach().clone(),
                # prompt_out,
                # pos_out,
                # neg_out,
                torch.as_tensor(pos_preds),
                torch.as_tensor(neg_preds),
                weighting=None)
